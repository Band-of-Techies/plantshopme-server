  ...
  products = await Promise.all(
    products.map(async (product) => {
      ...
      return {
        ...product._doc,
        dimensions,
        price: lowestPrice, // Add the lowest price to the product
        flashSaleInfo: flashSaleInfo
          ? {
              ...flashSaleInfo.toObject(),
              Status: flashSaleInfo.Status || 'Unlimited', // Include Status field with a default value if not present
            }
          : null,
        flashState,
      };
...
router.put('/updateProduct/:id', async (req, res) => {
  try {
    ...
    const { title, description, maincategory, category, subcategory, stock, FeatureTag, length, Pots, careType, careName, caredes, price, currency } = req.body;
