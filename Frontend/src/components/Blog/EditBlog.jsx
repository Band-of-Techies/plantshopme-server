...
const EditBlog = () => {
    ...
    useEffect(() => {
      const fetchBlog = async () => {
        try {
          const resp = await axios.get(`${process.env.REACT_APP_BASE_URL}/api/post/${id}`);
          setBlog(resp.data);
    ...
    const handleSubmit = async (e) => {
      ...
      try {
        const resp = await axios.put(`${process.env.REACT_APP_BASE_URL}/api/post`, data);
  ...
  const customStyles = {
    ...
    return(
       ...
       <MainSection>
        ...
        <h4>Edit blog</h4>
        <button className='btn btn-cancel' onClick={()=>handleCancel(id)}>Back</button>
        </div>
            ...
            <button className='btn btn-post' onClick={()=>handleSubmit()}>UPDATE POST</button>
