...
router.post('/selectedPlantcare', async (req, res) => {
  try {
    ...
    // Check if photo is null or not provided
    let photoUrl;
    if (!photo) {
      // Retrieve the corresponding photo from PlantCare collection based on careName
      const plantCareData = await PlantCare.findOne({ name: careName });
      if (!plantCareData) {
        return res.status(404).json({ error: 'Photo not found for the given careName.' });
      }
      photoUrl = plantCareData.photo.url;
    } else {
      photoUrl = photo;
    }
    ...
    if (existingPlantCare) {
      // If a row with the same title and careName exists, provide an error message
      return res.status(400).json({ error: 'Duplicate data: A product with the same title and careName already exists in the database.' });
    }
    ...
    const newPlantCare = new selectedPlantCare({
      ...
      category,
      subcategory,
      photo: photoUrl  // Assigning the photo URL
    });
